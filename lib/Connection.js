...
Connection.prototype.append = function(data, options, cb) {
  ...
  if (options.date) {
    if (!isDate(options.date))
      ...
      cmd += ' "';
      cmd += options.date.getUTCDate();
      cmd += '-';
      cmd += utils.MONTHS[options.date.getUTCMonth()];
      cmd += '-';
      cmd += options.date.getUTCFullYear();
      cmd += ' ';
      cmd += ('0' + options.date.getUTCHours()).slice(-2);
      cmd += ':';
      cmd += ('0' + options.date.getUTCMinutes()).slice(-2);
      cmd += ':';
      cmd += ('0' + options.date.getUTCSeconds()).slice(-2);
      cmd += ' +0000';
      cmd += '"';
  }
  cmd += ' {';
